...
from PySide2.QtWidgets import QGraphicsView, QGraphicsScene, QGraphicsRectItem, QGraphicsItem, QGraphicsPixmapItem, QAction, QGraphicsSceneContextMenuEvent, QGraphicsSceneMouseEvent
from PySide2.QtGui import QBrush, QPen, QPixmap, Qt, QPainterPath, QColor, QPainter
from PySide2 import QtGui, QtWidgets, QtCore
from PySide2.QtCore import QRectF, QPointF
import xml.etree.ElementTree as et
import os

class SlotImage(QGraphicsRectItem):
    ...
    def __init__(self, iface, slot_id, positionX, positionY, width, height, file_name):
        ...
        if(self.file_exist == True):
            self.pixmapFile = QPixmap(file_name)
    ...
    def mouseDoubleClickEvent(self, event: QtWidgets.QGraphicsSceneMouseEvent) -> None:
        ...
        change_image = QAction("Change image")
        change_image.triggered.connect(lambda: self.change_image())
        change_image.setIcon(QtGui.QIcon("resources/icons/image.png"))
    ...
    def saveReactValue(self):
        ...
        print('IMAGE: id: {0}, x: {1}, y: {2}'.format(self.id, self.pos().x(), self.pos().y()))
    ...
    def paint(self, painter, option, widget=None):
        ...
        if(self.iface.image_view_activate == False or self.file_exist == False):
            self.pixmapFile2 = QPixmap('resources/icons/image_not_availble.jpg')
            self.pixmapFile2 = self.pixmapFile2.scaled(float(self.slot_width), float(self.slot_height), aspectMode=Qt.IgnoreAspectRatio)
            painter.drawPixmap(QPointF(0, 0), self.pixmapFile2)
            painter.drawText(self.rect(), self.fileName)
        else:
            painter.drawPixmap(QPointF(0, 0), self.pixmapFile)
